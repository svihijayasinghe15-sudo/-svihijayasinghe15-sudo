
# SILUNYA JAYASINGHE

# Hi, I'm Silunya Jayasinghe 👋

🔐 Cybersecurity Undergraduate | 💻 Software Developer 

I'm a Cybersecurity undergraduate passionate about protecting digital systems, exploring security technologies, and building secure software solutions. While cybersecurity is my primary focus, I also enjoy software engineering and web development, allowing me to approach problems from both the security and development perspectives.

## 🚀 Skills & Technologies

### Cybersecurity
- Security Fundamentals
- Incident Response
- Web Security
- Vulnerability Assessment
- Capture The Flag (CTF)
- Secure Software Development

### Programming & Development
- Java
- Python
- JavaScript
- SQL
- HTML
- CSS
- JSP
- Java EE

### Tools & Platforms
- Git
- GitHub
- Linux
- MySQL
- IntelliJ IDEA
- VS Code
- AWS

## 🏆 Certifications & Achievements

### AWS Training & Certification
- AWS Security Fundamentals
- AWS Security Incident Response Overview
- Beyond Monitoring: Building Agentic SOC with Amazon Q

### Google
- Gemini Certified Student

### Achievements
- Top Performer – CRYPT 2.0 CTF Competition (Distinction)

## 💻 Featured Projects

### Web-Based Bill and Event Reminder System
> Software Engineering group project focused on event management and reminder notifications.

### Online Examination and Result Management System
> Web application developed using Java EE, JSP, HTML, CSS, and Tailwind.

### Inventory and Stock Management System
> Web-based inventory management solution.

## 🌐 Connect With Me

- LinkedIn: www.linkedin.com/in/silunya-jayasinghe-387060277
- GitHub: https://github.com/svihijayasinghe15-sudo?tab=repositories

## 📫 Open to Opportunities

I'm open to remote internships, cybersecurity opportunities, software engineering roles, and collaborative projects where I can continue developing my technical and security expertise.

---
⭐ Feel free to explore my repositories and connect with me!

